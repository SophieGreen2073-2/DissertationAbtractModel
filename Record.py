...
class RecordTime():
    def record_time_elapsed(self, num_robots, time_elapsed, uav_params):
        with open('dissertation_time_record_Elios_full_comms_utility.csv', 'a', newline='') as f:
            writer = csv.writer(f)
...
class RecordRedundancy():
    def record_overlap(self, overlap_area, numUAVs, uav_params):
        with open('dissertation_redundancy_record_Elios_full_comms_utility.csv', 'a') as f:
            writer = csv.writer(f)
